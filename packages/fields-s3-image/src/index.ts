...
async function inputResolver(data: ImageFieldInputType, context: KeystoneContext) {
  ...
  if (data.ref) {
    ...
    return getDataFromRef(data.ref);
  }
  if (!data.upload) {
    throw new Error('Either ref or upload must be passed to ImageFieldInput');
  }
  return getDataFromStream((await data.upload).createReadStream());
}
...
export const s3Image =
  <TGeneratedListTypes extends BaseGeneratedListTypes>(
    { isRequired, defaultValue, s3Config, ...config }: ImageFieldConfig<TGeneratedListTypes> = {}
  ): FieldTypeFunc =>
  () => {
