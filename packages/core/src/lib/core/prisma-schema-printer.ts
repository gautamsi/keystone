...
export function printPrismaSchema(config: KeystoneConfig, lists: Record<string, InitialisedList>) {
  const { prismaClientPath, provider, extendPrismaSchema: extendPrismaCompleteSchema, usePrismaNativeClient } = config.db

  const defaultOutputPath = usePrismaNativeClient ? null : 'node_modules/.prisma'
  const outputPath = prismaClientPath === '@prisma/client' ? defaultOutputPath : prismaClientPath
